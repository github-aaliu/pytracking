...
class siamDActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        thw_losses_test = [self.objective['iou'](s, data['gt_off']) for s in iou_pred]

        # Loss of the final filter
        thw_loss_test = thw_losses_test[-1]
        loss_target_thw = self.loss_weight['test_thw'] * thw_loss_test


        # Loss for the initial filter iteration
        loss_test_init_thw = 0
        if 'test_init_thw' in self.loss_weight.keys():
            loss_test_init_thw = self.loss_weight['test_init_thw'] * thw_losses_test[0]

        # Loss for the intermediate filter iterations
        loss_test_iter_thw = 0
        if 'test_iter_thw' in self.loss_weight.keys():
            test_iter_thw_weights = self.loss_weight['test_iter_thw']
            if isinstance(test_iter_thw_weights, list):
                loss_test_iter_thw = sum([a*b for a, b in zip(test_iter_thw_weights, thw_losses_test[1:-1])])
            else:
                loss_test_iter_thw = (test_iter_thw_weights / (len(thw_losses_test) - 2)) * sum(thw_losses_test[1:-1])



        # Total loss
        loss = loss_target_classifier + loss_test_init_clf + loss_test_iter_clf+loss_target_thw+loss_test_init_thw+loss_test_iter_thw

        # Log stats
        stats = {'Loss/total': loss.item(),
                 'Loss/target_clf': loss_target_classifier.item(),
                 'Loss/thw':loss_target_thw.item()}
        if 'test_init_clf' in self.loss_weight.keys():
            stats['Loss/test_init_clf'] = loss_test_init_clf.item()
        if 'test_iter_clf' in self.loss_weight.keys():
            stats['Loss/test_iter_clf'] = loss_test_iter_clf.item()
