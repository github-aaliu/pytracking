                                                       ...
                                                       scalar_t *data_col)
{
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    const int b_col = index / width_col / height_col/num_channels;
    ...
    const int w_in = w_col * stride_w ;
     ...
     scalar_t *data_col_ptr = data_col + (((c_im * batch_size + b_col) * height + h_in) * width + w_in)* kernel_h * kernel_w;
    const scalar_t *data_im_ptr = data_im + (b_col * num_channels + c_im) * height * width;
    const scalar_t *data_offset_ptr = data_offset + (b_col * deformable_group + deformable_group_index) * 2 * kernel_h * kernel_w;

    for (int i = 0; i < kernel_h; ++i)
    {
      for (int j = 0; j < kernel_w; ++j)
      {
        const int data_offset_h_ptr = i * kernel_w + j;
        const int data_offset_w_ptr = kernel_h*kernel_w+i * kernel_w + j;
        const scalar_t offset_h = data_offset_ptr[data_offset_h_ptr];
        const scalar_t offset_w = data_offset_ptr[data_offset_w_ptr];
        scalar_t val = static_cast<scalar_t>(0);
        const scalar_t h_im = h_in + offset_h;
        const scalar_t w_im = w_in + offset_w;
        if (h_im > -pad_h && w_im > -pad_w && h_im < height-1+pad_h && w_im < width-1+pad_w)
        {
                                                       ...
                                                       scalar_t *grad_im)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    const int j = (index / width_col / height_col / batch_size / channels) % kernel_w;
    const int i = index / width_col / height_col / batch_size / channels / kernel_w;
    ...
    const scalar_t *data_offset_ptr = data_offset + (b * deformable_group + deformable_group_index) * 2 * kernel_h * kernel_w ;
    const int data_offset_h_ptr = i * kernel_w + j;
    const int data_offset_w_ptr = kernel_h * kernel_w + i * kernel_w + j;
    const scalar_t offset_h = data_offset_ptr[data_offset_h_ptr];
    const scalar_t offset_w = data_offset_ptr[data_offset_w_ptr];
    const scalar_t cur_inv_h_data = h_in + offset_h;
    const scalar_t cur_inv_w_data = w_in + offset_w;
