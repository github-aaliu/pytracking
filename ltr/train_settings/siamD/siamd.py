...
from ltr.models.tracking import siamDnet
...
def run(settings):
    ...
    settings.num_frame = 3
    settings.boxes_per_frame = 4
    ...
    proposal_params = {'hw_factor': [0.9,1.1],  'area_factor': [0.9,1.1], 'boxes_per_frame': settings.boxes_per_frame}
    label_params = {'feature_sz': settings.feature_sz, 'sigma_factor': output_sigma, 'kernel_sz': settings.target_filter_sz}
    data_processing_train = processing.siamDProcessing(search_area_factor=settings.search_area_factor,
                                                      output_sz=settings.output_sz,
                                                      center_jitter_factor=settings.center_jitter_factor,
                                                      scale_jitter_factor=settings.scale_jitter_factor,
                                                      threshold = settings.hinge_threshold,
    ...
    data_processing_val = processing.siamDProcessing(search_area_factor=settings.search_area_factor,
                                                      output_sz=settings.output_sz,
                                                      center_jitter_factor=settings.center_jitter_factor,
                                                      scale_jitter_factor=settings.scale_jitter_factor,
                                                      threshold = settings.hinge_threshold,
    ...
    dataset_train = sampler.DiMPSampler([lasot_train,got10k_train,coco_train], [0.5,1,1],
                                        samples_per_epoch=26000, max_gap=30, num_test_frames=settings.num_frame,
                                        num_train_frames=settings.num_frame, processing=data_processing_train)

    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=settings.batch_size, num_workers=settings.num_workers,
                             shuffle=False, drop_last=True, stack_dim=1)
    """
    ...
    net = siamDnet.siamdnet18(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    objective = {'iou': ltr_losses.thw_loss(), 'test_clf': ltr_losses.LBHinge(threshold=settings.hinge_threshold)}

    loss_weight = {'test_thw': 0.0001, 'test_init_thw': 0.0001, 'test_iter_thw': 0.0004, 'test_clf': 100, 'test_init_clf': 100, 'test_iter_clf': 400}

    actor = actors.siamDActor(net=net, objective=objective, loss_weight=loss_weight)
