...
from ltr.models.tracking import cttnet
...
def run(settings):
    ...
    label_params = {'feature_sz': settings.feature_sz, 'sigma_factor': output_sigma, 'kernel_sz': settings.filter_sz}
    ...
    dataset_train = sampler.DiMPSampler([coco_train], [1],
    ...
    net = cttnet.cttnet(n_stack = settings.nstack, kernel_size=settings.filter_sz)
    ...
    actor = actors.CttActor(net=net, objective=objective, loss_weight=loss_weight)
