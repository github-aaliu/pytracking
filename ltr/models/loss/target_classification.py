...
class thw_loss(nn.Module):
    ...
    args:
        ...
        clip:  Clip the loss if it is above this value.
    """
    def __init__(self, error_metric=nn.SmoothL1Loss(reduction='sum'), threshold=None, clip=None):
        super().__init__()
        self.error_metric = error_metric
        self.threshold = threshold if threshold is not None else -100
        self.clip = clip
    ...
    def forward(self, prediction, label, target_bb=None):
        positive_mask = (label > 0).float()
        prediction= positive_mask*prediction
